...
public final class WebDriverBackedEmbeddedBrowser implements EmbeddedBrowser {
	...
	private Map<Pattern, String> regexFilters;
	...
	private WebDriverBackedEmbeddedBrowser(WebDriver driver,
			List<String> filterAttributes, Map<Pattern, String> regexFilters,
	...
	private WebDriverBackedEmbeddedBrowser(WebDriver driver,
		...
		this(driver, filterAttributes, regexFilters, crawlWaitReload,
				crawlWaitEvent);
		this.ignoreFrameChecker = ignoreFrameChecker;
	}
	...
	public static WebDriverBackedEmbeddedBrowser withRemoteDriver(
			String hubUrl, List<String> filterAttributes, long crawlWaitEvent,
			long crawlWaitReload) {
		return WebDriverBackedEmbeddedBrowser.withDriver(
				buildRemoteWebDriver(hubUrl), filterAttributes, crawlWaitEvent,
				crawlWaitReload);
	}

	public static WebDriverBackedEmbeddedBrowser withRemoteDriver(
			String hubUrl, List<String> filterAttributes,
			Map<Pattern, String> regexFilters, long crawlWaitEvent,
			long crawlWaitReload) {
		return WebDriverBackedEmbeddedBrowser.withDriver(
				buildRemoteWebDriver(hubUrl), filterAttributes, regexFilters,
	...
	public static WebDriverBackedEmbeddedBrowser withDriver(WebDriver driver,
			List<String> filterAttributes, long crawlWaitEvent,
			long crawlWaitReload) {
		return new WebDriverBackedEmbeddedBrowser(driver, filterAttributes,
				crawlWaitEvent, crawlWaitReload);
	}
	...
	public static WebDriverBackedEmbeddedBrowser withDriver(WebDriver driver,
		...
		return new WebDriverBackedEmbeddedBrowser(driver, filterAttributes,
				regexFilters, crawlWaitEvent, crawlWaitReload);
	}
	...
	public static WebDriverBackedEmbeddedBrowser withRemoteDriver(String hubUrl) {
		return WebDriverBackedEmbeddedBrowser
				.withDriver(buildRemoteWebDriver(hubUrl));
	}
	...
	private static RemoteWebDriver buildRemoteWebDriver(String hubUrl) {
		...
		try {
			...
			LOGGER.error(
					"The given hub url of the remote server is malformed can not continue!",
					e);
			return null;
		}
	...
	public void goToUrl(String url) {
		try {
			...
			LOGGER.error(
					"goToUrl got interrupted while waiting for the page to be loaded",
					e);
			return;
		}
	}
	...
	public String getDom() {
		...
		try {
			String dom = toUniformDOM(Helper
	...
	private String toUniformDOM(String html) {
		...
		// TODO (Stefan), Following lines are a serious performance bottle
		// neck...
		...
		htmlFormatted = filterGeneral(htmlFormatted);
	...
	private String filterGeneral(String html) {
		String retval = html;
		//if (regexFilters.size() == 0)
		//	System.err.println("NO FILTERS!");

		for (Pattern p : regexFilters.keySet()) {
			Matcher m = p.matcher(retval);
			retval = m.replaceAll(regexFilters.get(p));
			// if(!html.equals(retval))
			// System.err.println("Made RegEx replacement '"+regexFilters.get(p)+"'.");
			/*
			 * else {
			 * System.err.println("No replacement for '"+p.toString()+"'.");
			 * 
			 * System.err.print("Find <TITLE>:"+Integer.toString(retval.indexOf(
			 * "<TITLE>")));
			 * System.err.println(";  Find </TITLE>:"+Integer.toString
			 * (retval.indexOf("</TITLE>")));
			 * 
			 * try { BufferedWriter bw2 = new BufferedWriter(new
			 * FileWriter("/srv/logger/states/errstates.log", true));
			 * bw2.write("--------------------------------------------------\n\n"
			 * +retval+"\n"); bw2.close(); } catch (IOException e) { // TODO
	...
	public boolean input(Identification identification, String text) {
		try {
			WebElement field = browser.findElement(identification
					.getWebDriverBy());
			if (field != null) {
		...
		try {
			...
			if (eventable.getRelatedFrame() != null
				...
				try {
					...
					LOGGER.debug(
							"Frame not found, possibily while back-tracking..",
							e);
					// TODO Stefan, This exception is catched to prevent
					// stopping from working
					// This was the case on the Gmail case; find out if not
					// switching (catching)
	...
	private void appendFrameContent(Element orig, Document document,
		...
		for (int i = 0; i < nodeList.size(); i++) {
			...
			if (nameId != null
					&& !ignoreFrameChecker.isFrameIgnored(frameIdentification
							+ nameId)) {
	...
	/**
	 * @return the list of RegEx filters to be applied to the DOM.
	 */
	protected Map<Pattern, String> getRegexFilters() {
		return regexFilters;
	}
	...
	private void takeScreenShotOnBrowser(WebDriver driver, File file)
			throws CrawljaxException {
		if (driver instanceof TakesScreenshot) {
			...
			takeScreenShotOnBrowser(((WrapsDriver) driver).getWrappedDriver(),
					file);
		} else {
			throw new CrawljaxException(
					"Your current WebDriver doesn't support screenshots.");
		}
	}
	...
	public void updateConfiguration(CrawljaxConfigurationReader configuration) {
		...
		this.regexFilters = configuration.getRegexFilters();
		this.crawlWaitReload = configuration.getCrawlSpecificationReader()
				.getWaitAfterReloadUrl();
		this.crawlWaitEvent = configuration.getCrawlSpecificationReader()
				.getWaitAfterEvent();
