...
public final class CrawljaxPluginsUtil {
	...
	public static StateVertix runDetectCloneStatePlugins(StateVertix state) {
		LOGGER.info("Running DetectCloneStatePlugins...");
		StateVertix retval = null;
		for(Plugin plugin : CrawljaxPluginsUtil.PLUGINS) {
			if(plugin instanceof DetectCloneStatePlugin) {
				retval = ((DetectCloneStatePlugin) plugin).findCloneState(state);
				if(retval != null) {
					return retval; //Exit on first clone detected
				}
			}
		}
		return null; //Not found to be a clone
	}

	public static void runOnAddStateVertixPlugins(StateVertix newState, StateFlowGraph callingGraph) {
		LOGGER.info("Running OnAddStateVertixPlugins...");
		for (Plugin plugin : CrawljaxPluginsUtil.PLUGINS) {
			if (plugin instanceof OnAddStateVertixPlugin) {
				((OnAddStateVertixPlugin) plugin).onAddVertix(newState, callingGraph);
			}
		}
	}
