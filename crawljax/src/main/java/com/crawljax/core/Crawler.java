...
public class Crawler implements Runnable {
	...
	private ClickResult clickTag(final Eventable eventable) throws CrawljaxException {
		...
		if (this.fireEvent(eventable)) {
			StateVertix newState =
					//Heck, we could just change ctor to only accept Crawler
			        new StateVertix(getBrowser().getCurrentUrl(), controller.getSession()
			                .getStateFlowGraph().getNewStateName(), this);
	...
	private boolean crawl() throws CrawljaxException {
		...
		orrigionalState.runPreStateCrawlingPlugins(controller.getSession());
	...
	/**
	 * Add for state identification use
	 * @return the controller
	 */
	public CrawljaxController getController() {
		return controller;
	}
	
	/**
	 * @return the candidateExtractor
	 */
	public CandidateElementExtractor getCandidateExtractor() {
